...
function printHelp() {
    ...
    echo "      - 'generate-images' - Pull and update Fabric images to run in Containernet"
...
upgradeDockerImages() {
  ...
  if [ "${NODOCKER}" == 0 ]; then
    for IMAGES in peer; do
        ...
        docker commit --change "CMD /bin/bash" $IMAGES hyperledger/fabric-$IMAGES:$TAG.1
