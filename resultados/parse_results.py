...
if __name__ == "__main__":
    ...
    for key, value in avg.items():
        ...
        calculared_avg[key]['write']['avg'] = np.mean([c['av'] for c in value['write']])
        calculared_avg[key]['read']['avg'] = np.mean([c['av'] for c in value['read']])
        calculared_avg[key]['write']['std'] = np.std([c['av'] for c in value['write']])
        calculared_avg[key]['read']['std'] = np.std([c['av'] for c in value['read']])
        calculared_avg[key]['write']['max'] = np.max([c['av'] for c in value['write']])
        calculared_avg[key]['read']['max'] = np.max([c['av'] for c in value['read']])
        calculared_avg[key]['write']['min'] = np.min([c['av'] for c in value['write']])
        calculared_avg[key]['read']['min'] = np.min([c['av'] for c in value['read']])
